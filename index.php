...
//hurl's
#$esql=1;
 if (isset($_GET["categoryID"]) && CONF_CHPU && db_r('select hurl from '.CATEGORIES_TABLE.' where categoryID='.(int)$_GET["categoryID"]))
   include_once('./core/core_404.php');
 if (isset($_GET["productID"]) && CONF_CHPU && db_r('select hurl from '.PRODUCTS_TABLE.' where productID='.(int)$_GET["productID"]))
   include_once('./core/core_404.php');
