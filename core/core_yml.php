<?php
	...
	echo "\t<platform>Shop-Script LegoSP</platform>\n";
	echo "\t<version>6.1</version>\n";
	echo "\t<email>legoedition@gmail.com</email>\n";
	echo "\t<currencies>\n\t\t<currency id=\"RUR\" rate=\"1\"/>\n\t</currencies>\n";
	echo "\t<categories>\n";
	$catid='';
	$q = db_query("SELECT categoryID, name, parent, products_count FROM ".CATEGORIES_TABLE." where categoryID<>0 and enabled=1 ORDER BY name") or die (db_error());	
	while ($row = db_fetch_row($q))
	{
		echo  "\t\t<category id=\"".$row[0]."\" parentId=\"".$row[2]."\">".formspecialchars($row[1])."</category>\n";
		$catid .=$row[0].','; 
	}
