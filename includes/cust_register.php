<?php
          if (isset($custID) && ($custID==0) )
          {
            ...
            if (!isset($authData['email'])) $authData['email']='';
            $smarty->assign('email',$authData['email']);
	...
	elseif (isset($_POST["customer_register"]))
	{
		...
		$q=db_query("SELECT count(custID) FROM ".CUST_TABLE." WHERE (cust_email='".$email."')") or die (db_error());
		$row = mysql_fetch_array($q);
		if($row[0]>0) // customer email already exists
		{
		...
		{   
			...
			$file_name = ROOT_DIR."/css/css_".CONF_COLOR_SCHEME."/image/mail_logo.jpg";
                        ...
                        { 
                          ...
                          $NOTIFICATION_SUBJECT=win2utf(EMAIL_CUSTOMER_REGISTER_NOTIFICATION_SUBJECT);
                        }   
			...
			$html_body = $smarty_mail->fetch("register_notification.tpl.html");
                        if (DB_CHARSET!='cp1251') $html_body = win2utf($html_body);
			$body = "--$bound\n";
