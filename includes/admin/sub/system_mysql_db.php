	...
	if (!strcmp($sub, "mysql_db"))
	{
		...
		if (isset($_FILES['mysql_db_import']) && $_FILES['mysql_db_import']['name']) //import db
		{
			...
			//create tables
				...
				db_query("DROP TABLE IF EXISTS `ssme_ordered_carts`") or die (db_error());
				...
				db_query("CREATE TABLE `ssme_ordered_carts` (productID INT NOT NULL, orderID INT NOT NULL, name CHAR(255), Price FLOAT, Quantity INT, PRIMARY KEY (productID, orderID)) CHARACTER SET cp1251 COLLATE cp1251_general_ci") or die (db_error());
			...
			$q = db_query("SELECT * FROM `ssme_ordered_carts` WHERE 1") or die (db_error());
			while ($row = db_fetch_row($q))
				...
				if ($row[11] != '')
				  {
					...
					if (file_exists("./products_pictures/".$row[11])) unlink("./products_pictures/".$row[11]);
				  ...
				  {
					if (file_exists("./products_pictures/".$row[6])) unlink("./products_pictures/".$row[6]);
					if (file_exists("./products_pictures/".$row[8])) unlink("./products_pictures/".$row[8]);
					if (file_exists("./products_pictures/".$row[11])) unlink("./products_pictures/".$row[11]);
					$row[6] = '';
					$row[8] = '';
					$row[11] = '';
			...
			else  //standart import			
			{
				...
				for ($i=1; $i<count($f); $i++)
				{       
                                        if (DB_CHARSET!='cp1251') $f[$i]=win2utf($f[$i]);
		...
		if (isset($_GET["optimize"])) //show successful save confirmation message
		{
			...
			while ($row = db_fetch_row($q)) $result .= "<tr><td>$row[0]</td><td>$row[1]</td><td>$row[2]</td><td>$row[3]</td></tr>";
			  ...
			  $data .= $insertStatement;
			   ...
			   $data .= "\n"; 
				...
				while ($row = db_fetch_row($q))
					...
					$p=implode(", ",$row);
					$insertStatement .=" (".$p.")";
                                        echo count($row).'||i='.$i."<br>";  
                                        if (count($row)>($i+1)) $insertStatement .=",\n"; else  $insertStatement .=";\n";
                                       	fputs($f,$insertStatement);
					}
				}
                       */
                        }   
